...
class WebhookAckResponse(BaseModel):
    ...
    built for handoff to the ingestion use case (wired in a later change).
    `status="ignored"` means the event was recognized but intentionally
    dropped (wrong inbox, private note, non-incoming message type) — this is
    NOT an error, per spec: dropped events must still be acknowledged.
    """
